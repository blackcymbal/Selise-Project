export { default as CourseCardPreview } from "./course-card-details-preview/CourseCardPreview";
export { default as CourseDetailsCurriculum } from "./CourseDetailsCurriculum";
export { default as CourseDetailsContact } from "./CourseDetailsContact";
export { default as CourseDetailsFaq } from "./CourseDetailsFaq";
export { default as CourseDetailsInstructors } from "./course-details-instructor/CourseDetailsInstructors";
export { default as CourseDetailsPaymentProcess } from "./CourseDetailsPaymentProcess";
export { default as CourseDetailsPreRequisites } from "./CourseDetailsPreRequisites";
export { default as CourseDetailsPurchase } from "./CourseDetailsPurchase";
export { default as CourseDetailsReviews } from "./course-details-review/CourseDetailsReviews";
export { default as CourseDetailsTopBar } from "./CourseDetailsTopBar";

export { default as CourseLearningOutcomes } from "./CourseLearningOutcomes";
export { default as CoursePromo } from "./CoursePromo";
export { default as CourseRequirements } from "./CourseRequirements";
export { default as MyCourseCard } from "./MyCourseCard";
